...
export function RaffleBuyTabs({ raffleId, currency, totalTickets, unitPriceCents, paymentInfo, allowManual = true, isFree = false, disabledAll = false }: { raffleId: string; currency: string; totalTickets: number; unitPriceCents: number; paymentInfo: RafflePaymentInfo | null; allowManual?: boolean; isFree?: boolean; disabledAll?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!allowManual || tab === 'quick' ? (
        <RaffleQuickBuy raffleId={raffleId} currency={currency} totalTickets={totalTickets} unitPriceCents={unitPriceCents} paymentInfo={paymentInfo ?? undefined} isFree={isFree} disabledAll={disabledAll} />
      ) : (
        <div className="space-y-4">
          ...
          <RaffleBuySection raffleId={raffleId} currency={currency} unitPriceCents={unitPriceCents} paymentInfo={paymentInfo} isFree={isFree} disabledAll={disabledAll}
          />
        </div>
      )}
    </div>
