...
export async function getRaffle(id: string): Promise<Raffle | null> {
  ...
  const trySelect = async (withAllow: boolean) => {
    const fields = withAllow
      ? 'id,name,description,status,currency,ticket_price_cents,prize_amount_cents,top_buyer_prize_cents,image_url,total_tickets,allow_manual'
      : 'id,name,description,status,currency,ticket_price_cents,prize_amount_cents,top_buyer_prize_cents,image_url,total_tickets';
