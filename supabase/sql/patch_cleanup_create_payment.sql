-- Patch: cleanup de funciones duplicadas create_payment_for_session
-- Objetivo: dejar solo la versión con instagram y search_path fijo
BEGIN;

-- 1) Inspección (ejecutar aparte si quieres ver antes de dropear)
-- SELECT p.oid, p.proname, pg_get_function_identity_arguments(p.oid) AS args,
--        (SELECT array_to_string(p.proconfig,';')) AS config, p.prosecdef
-- FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
-- WHERE n.nspname='public' AND p.proname='create_payment_for_session';

-- 2) Eliminar posibles firmas antiguas sin instagram o sin search_path
...
-- 3) Re-crear solo la versión actual (14 args, incluye instagram) con search_path
...
-- Verificación post:
-- SELECT p.proname, pg_get_function_identity_arguments(p.oid) AS args, array_to_string(p.proconfig,';') AS config
-- FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
-- WHERE n.nspname='public' AND p.proname='create_payment_for_session';
